...
public class Tuonti
{
   ...
   private void haeHarrastajat(Sheet välilehti, Map<Integer, Perhe> perheet, Tuontitulos tulos)
   {
      for (int rivi = 2; rivi < välilehti.getRows(); rivi++)
      {
         ...
         asetaSopimukset(välilehti, rivi, harrastaja, tulos);
         tulos.lisääHarrastaja(harrastaja);
      }
   }

   private void asetaSopimukset(Sheet välilehti, int rivi, Harrastaja harrastaja, Tuontitulos tulos)
   {
      if (sopimustyypit == null)
      {
         return;
      }
      teeJäsensopimus(harrastaja, tulos);
      teeHarjoittelusopimus(välilehti, rivi, harrastaja, tulos);
      if (!harrastaja.isSopimuksetOK()) {
         harrastaja.lisääHuomautus("Tarkista sopimukset");
      }
   }

   private void teeHarjoittelusopimus(Sheet välilehti, int rivi, Harrastaja harrastaja, Tuontitulos tulos)
   {
   ...
   private void teeJäsensopimus(Harrastaja harrastaja, Tuontitulos tulos)
   {
      Sopimus sopimus = new Sopimus();
      Optional<Sopimustyyppi> jäsenmaksu = sopimustyypit.stream().filter(s -> s.isJäsenmaksutyyppi()).findFirst();
      if (!jäsenmaksu.isPresent())
      {
         tulos.lisääVirhe("Jäsenmaksutyyppi ei löytynyt");
         return;
      }
      sopimus.setTyyppi(jäsenmaksu.get());
      harrastaja.lisääSopimus(sopimus);
   }
