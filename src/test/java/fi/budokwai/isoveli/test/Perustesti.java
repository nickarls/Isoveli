...
public class Perustesti
{
   ...
   protected Vyˆkoe teeVyˆkoe(Harrastaja harrastaja, String koska, String vyˆarvoNimi, int j‰rjestys, boolean poom,
      boolean dan)
   {
      Vyˆkoe vyˆkoe = new Vyˆkoe();
      Vyˆarvo vyˆarvo = new Vyˆarvo();
      vyˆarvo.setId(j‰rjestys);
      vyˆarvo.setNimi(vyˆarvoNimi);
      vyˆarvo.setJ‰rjestys(j‰rjestys);
      vyˆarvo.setPoom(poom);
      vyˆarvo.setDan(dan);
      ...
      harrastaja.lis‰‰Vyˆkoe(vyˆkoe);
      return vyˆkoe;
   }

   protected Vyˆarvo teeVyˆarvo(String arvo)
   {
      return entityManager.createQuery("select v from Vyˆarvo v where v.nimi=:nimi", Vyˆarvo.class)
         .setParameter("nimi", arvo).getSingleResult();
   }

   protected Vyˆkoe teeVyˆkoe(Harrastaja harrastaja, String koska, String vyˆarvoNimi, int j‰rjestys)
   {
      return teeVyˆkoe(harrastaja, koska, vyˆarvoNimi, j‰rjestys, false, false);
   }
